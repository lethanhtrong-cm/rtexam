...
import { collection, addDoc, updateDoc, deleteDoc, doc, setDoc, onSnapshot, serverTimestamp, query, orderBy, getDocs } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
function updateCategorySelectOptions() {
    const dataHtml = UI.renderSidebarAndSelect(adminCategoryTree, currentAdminCategory);
...
function loadPptxData() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            allAdminPptx.push({ id: docSnap.id, ...docSnap.data() });
        });
        ...
        if (!isReordering) renderAdminTable(); 
    }, (error) => {
