...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        ...
        .form-group input, .form-group select { width: 100%; padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px; outline: none; font-size: 1rem; background: white; }
        ...
        td.link-cell { max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #64748b; font-size: 0.9rem; }
        .btn-action { border: none; padding: 8px 12px; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 0.85rem; color: white; }
        .btn-comment { background: #3b82f6; margin-right: 5px; } 
        .btn-reset { background: #64748b; margin-right: 5px; }
...
<body>
    <div class="admin-container">
        ...
        <div class="admin-layout">
            <!-- Sidebar bây giờ sẽ trống và chờ JS nhúng vào -->
            <div class="admin-sidebar" id="dynamic-admin-sidebar">
                <h3>Danh mục quản lý</h3>
                <div style="padding: 20px; text-align: center; color: #64748b;"><i class="fa-solid fa-spinner fa-spin"></i> Đang tải...</div>
            </div>
            ...
            <div class="admin-main">
                <div style="background: #f8fafc; padding: 25px; border-radius: 8px; border: 1px dashed #cbd5e1; margin-bottom: 30px;">
                    ...
                    <div class="form-group">
                        ...
                        <!-- Select Option cũng sẽ trống và chờ JS nhúng vào -->
                        <select id="pptx-category">
                        </select>
                    </div>
                ...
                <table>
                    ...
                    <tbody id="pptx-tbody">
                        <tr><td colspan="5" style="text-align: center; color: #64748b;">Đang tải dữ liệu...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <!-- Modal Bình luận hiện có -->
    <div id="comments-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.6); z-index: 1000; align-items: center; justify-content: center; backdrop-filter: blur(4px);">
        <div style="background: white; width: 90%; max-width: 600px; border-radius: 12px; padding: 25px; max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 15px 35px rgba(0,0,0,0.15);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; border-bottom: 1px solid #e2e8f0; padding-bottom: 15px;">
                <h2 style="font-size: 1.2rem; color: #0f172a; margin: 0;"><i class="fa-solid fa-comments"></i> Bình luận: <span id="modal-lecture-title" style="color: #3b82f6;"></span></h2>
                <button id="close-comments-modal" style="background: #f1f5f9; border: none; width: 35px; height: 35px; border-radius: 50%; font-size: 1.2rem; cursor: pointer; color: #64748b; transition: 0.2s;"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <div id="modal-comments-list" style="overflow-y: auto; flex: 1; display: flex; flex-direction: column; gap: 15px; padding-right: 5px;">
            </div>
        </div>
    </div>

    <!-- THÊM MỚI: Modal Cấu hình Cây thư mục -->
    <div id="tree-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.6); z-index: 1000; align-items: center; justify-content: center; backdrop-filter: blur(4px);">
        <div style="background: white; width: 90%; max-width: 900px; border-radius: 12px; padding: 25px; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 15px 35px rgba(0,0,0,0.15);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; border-bottom: 1px solid #e2e8f0; padding-bottom: 15px;">
                <h2 style="font-size: 1.2rem; color: #0f172a; margin: 0;"><i class="fa-solid fa-sitemap"></i> Cấu hình Cây Thư mục (Hiển thị User)</h2>
                <button id="close-tree-modal" style="background: #f1f5f9; border: none; width: 35px; height: 35px; border-radius: 50%; font-size: 1.2rem; cursor: pointer; color: #64748b; transition: 0.2s;"><i class="fa-solid fa-xmark"></i></button>
            </div>
            
            <div id="tree-editor-container" style="flex: 1; overflow-y: auto; padding-right: 10px;">
                <!-- JS sẽ render form vào đây -->
            </div>
            
            <div style="text-align: right; border-top: 1px solid #e2e8f0; padding-top: 15px; margin-top: 15px;">
                <button onclick="window.addMainCat()" class="btn-submit" style="background: #10b981; margin-right: 10px;"><i class="fa-solid fa-plus"></i> Thêm Chuyên Khoa Gốc</button>
                <button id="btn-save-tree" class="btn-submit"><i class="fa-solid fa-floppy-disk"></i> Lưu & Áp Dụng Ngay</button>
            </div>
        </div>
    </div>
